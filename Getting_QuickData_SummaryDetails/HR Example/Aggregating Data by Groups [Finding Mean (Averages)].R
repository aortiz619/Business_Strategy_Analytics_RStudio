...
# Now lets more information about our variables like database minimum, mean, maximum and quantities
summary(data)
...
#we create a variable called promptemp which computes the number of observation
...
# To view the table we type view(aggdata) to the command line
View(aggdata)
...
# Hierarchical Clustering using Ward criterion
hcward = hclust(d, method ="ward.D")
...
# Remember that here the aggregate data table, will populate 4 groups with the headers we wanted
# Each result under each variable is to be know as a Mean (where data look at the sheet and found 4 groups each sharing a similar average)
aggdata
...
# Instead of write.csv, you can also use write.csv2() if you encounter an error due to regional setting for separators
